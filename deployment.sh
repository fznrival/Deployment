...
write_contract_and_script() {
    log_timestamp "${YELLOW}Menulis kontrak dan script deploy dengan nama token: $TOKEN_NAME (${TOKEN_SYMBOL})...${RESET}"
    ...
    if (process.env.SEND_TO_FILE === "y") {
      ...
      for (const recipient of recipients) {
        ...
        if (address.match(/^0x[a-fA-F0-9]{40}$/)) {
          console.log(`Sending 1000 tokens to address from file: ${address}`);
          const tx = await tokenAuthority.transfer(address, ethers.utils.parseEther("1000"));
          await tx.wait();
          console.log(`Transfer successful: ${tx.hash}`);
        } else {
    ...
    while [ $retry_count -lt $max_retries ]; do
        ...
        if DEPLOY_OUTPUT=$(npx hardhat run scripts/deploy.js --network custom 2>&1); then
            ...
            if [ "$SEND_TO_RANDOM" = "y" ]; then
                echo "$DEPLOY_OUTPUT" | while IFS= read -r line; do
                    if [[ "$line" =~ "Sending 1000 tokens to random address:" ]]; then
                        RANDOM_ADDRESS=$(echo "$line" | grep -oP 'Sending 1000 tokens to random address: \K(0x[a-fA-F0-9]{40})')
