...
if ! command -v jq &> /dev/null; then
    echo -e "${YELLOW}Menginstal jq...${RESET}"
...
write_contract_and_script() {
    log_timestamp "${YELLOW}Menulis kontrak ERC721 dan script deploy dengan nama token: $TOKEN_NAME (${TOKEN_SYMBOL})...${RESET}"

    # Bersihkan artifact lama untuk memastikan kompilasi ulang
    if [ -d "$SCRIPT_DIR/hardhat-project/artifacts" ]; then
        rm -rf "$SCRIPT_DIR/hardhat-project/artifacts"
        rm -rf "$SCRIPT_DIR/hardhat-project/cache"
    fi
...
contract TokenAuthority is ERC721 {
...
async function main() {
  ...
  console.log("Deploying ERC721 contracts with multiple wallets...");
  for (let i = 0; i < signers.length; i++) {
    ...
    // Debugging: Cetak fungsi yang tersedia
    console.log("Available functions on tokenAuthority:", Object.keys(tokenAuthority.functions));
    ...
    if (process.env.SEND_TO_RANDOM === "y") {
      ...
      console.log(`Sending token ID: 0 to random address: ${randomAddress}`);
      console.log(`Random private key: ${randomPrivateKey}`);
      
      // Panggil safeTransferFrom langsung tanpa variabel perantara
      const tx = await tokenAuthority.connect(wallet)["safeTransferFrom(address,address,uint256,bytes)"](
        wallet.address,
        randomAddress,
        0,
        "0x"
    ...
    if (process.env.SEND_TO_FILE === "y") {
      ...
      for (const recipient of recipients) {
        ...
        if (address.match(/^0x[a-fA-F0-9]{40}$/)) {
          console.log(`Sending token ID: 0 to address from file: ${address}`);
          const tx = await tokenAuthority.connect(wallet)["safeTransferFrom(address,address,uint256,bytes)"](
            wallet.address,
            address,
            0,
    ...
    while [ $retry_count -lt $max_retries ]; do
        ...
        if DEPLOY_OUTPUT=$(npx hardhat run scripts/deploy.js --network custom 2>&1); then
            ...
            if [ "$SEND_TO_RANDOM" = "y" ]; then
                echo "$DEPLOY_OUTPUT" | while IFS= read -r line; do
                    if [[ "$line" =~ "Sending token ID: 0 to random address:" ]]; then
                        RANDOM_ADDRESS=$(echo "$line" | grep -oP 'Sending token ID: 0 to random address: \K(0x[a-fA-F0-9]{40})')
